...
require_once 'vendor/autoload.php'; // require paypal files
...
class Paypal_center extends CI_Controller
{
    public $_api_context;
    ...
    function index(){
        ...
        //$this->template->load('frontend/front_end','content/payment_credit_form');
        $this->session->set_flashdata('fail','Payment failed');

        
        
        redirect('frontend/auth_login/centerpaid');
    }
...
// payment approval/ cancellation.
        ...
        $redirectUrls->setReturnUrl($baseUrl."Paypal_center/getPaymentStatus")
            ->setCancelUrl($baseUrl."Paypal_center/getPaymentStatus");
...
// the above types and intent set to sale 'sale'
        ...
        if ($result->getState() == 'approved') {
         ...
         $this->session->set_userdata($arraydata);
          ...
          // print_r($this->session->userdata);exit;
            ...
            $this->paypal->create($Total,$Subtotal,$Tax,$PaymentMethod,$PayerStatus,$PayerMail,$saleId,$CreateTime,$UpdateTime,$State,$cust_id,$payment_id);
            $this->session->set_flashdata('success_msg','Payment success');
            redirect('Paypal_center/success');
        }
        $this->session->set_flashdata('success_msg','Payment failed');
        redirect('Paypal_center/cancel');
    }
    function success(){
        // $this->load->view("content/success");
         $this->session->set_flashdata('succ1','Payment success');
         if ((!$this->ion_auth->logged_in())) {
                redirect('frontend/auth_login/centerpaid');
            }
            else{
                 redirect('frontend/auth_login/centerpaids');
            }
    ...
    function cancel(){
        ...
        $this->template->load('frontend/register/institution_register','content/cancel');
